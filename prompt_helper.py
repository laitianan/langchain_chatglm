...
def create_fun_prompt(param: Interface) -> PromptTemplate:
    response_schemas = [ResponseSchema(name=p.name, type=p.type, description=f'{p.title}，抽取不到时使用"None"代替') for
...
现在有一些意图，类别为：{intents}，
你扮演AI角色的任务是根据***分隔符的文本对话，通过理解或者user聊天的关键词模糊匹配，比如聊天记录含有业绩就匹配类别中所有含业绩该关键词所有意图，使用列表方式返回,比如 intention_name:["a","b"]。
输出应该是严格按以下模式格式化的标记代码片段，必须包括开头和结尾的" ' ' ' json"和" ' ' ' ":
```json
{{
